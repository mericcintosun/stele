        ...
        {logs.map((log) => (
          <li key={log.id} className="space-y-2 px-4 py-3">
            <div className="flex items-center gap-2">
              <span
                className={`rounded border px-1.5 py-0.5 font-mono text-[10px] font-semibold uppercase ${STAGE_STYLE[log.stage] ?? STAGE_STYLE.signal}`}
              >
                {log.stage}
              </span>
            ...
            {log.weexResponse ? (
              <p className="rounded border border-ok/30 bg-ok/5 px-2 py-1.5 font-mono text-[10px] text-ok">
